...
The automated workflow validates both package architectures, checksums and the release certificate, then creates a draft **stable** release. Publish the draft after these gates succeed. GitHub CI does not exercise the PowerToys UI/COM lifecycle, ARM64 runtime, or an upgrade with real user settings and secrets; those checks remain for field validation after installation. The [v0.0.5.5 release notes](release-notes-v0.0.5.5.md) list what was verified and what remains to check.
...
Build and validation run with read-only repository permissions; a separate publish job receives only the validated payload and has `contents: write`. It creates a draft marked as a prerelease so a maintainer can inspect the generated assets before making them public. Publishing the candidate keeps the stable `latest` release unchanged. The release cannot publish an extra file, a stale checksum or an MSIX with a different version, publisher, architecture or payload.
