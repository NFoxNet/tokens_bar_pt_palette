...
The GitHub release workflow repeats the package checks before publication and compares the output directory with an explicit allowlist. The release cannot publish an extra file, a stale checksum or an MSIX with a different version, publisher, architecture or signature.
