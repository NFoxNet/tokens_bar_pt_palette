...
- Для обновления подписанного Release MSIX устанавливайте новую версию поверх текущей с той же package identity; не удаляйте пакет перед обновлением, иначе можно потерять зашифрованные provider secrets. `Remove-AppxPackage -PreserveApplicationData` работает только для development-mode приложений, зарегистрированных из loose-file layout, и не поддерживается для установленного Release MSIX ([документация Microsoft](https://learn.microsoft.com/en-us/powershell/module/appx/remove-appxpackage)). Для локальной Debug-проверки сначала предпочитайте остановить extension process и зарегистрировать Debug `AppxManifest.xml` напрямую; подпись и сертификаты для этого не нужны. Перед переключением с подписанной установки на другой тип регистрации проверьте отдельную процедуру миграции и сохранения данных.
